 ...
 */
export function parseAnalyzerProgressLine(line: string): AnalyzerProgressEvent | null {
  const markerIndex = line.indexOf(PROGRESS_PREFIX);
  if (markerIndex < 0) return null;

  try {
    const value = JSON.parse(line.slice(markerIndex + PROGRESS_PREFIX.length)) as unknown;
