...
function runProcess(
  ...
  return new Promise((resolvePromise, reject) => {
    ...
    let stdout = '';
    let stderr = '';
    const forward = (chunk: Buffer, target: 'stdout' | 'stderr'): void => {
...
function commandExists(command: string): Promise<boolean> {
  ...
  return runProcess(checker, [command]).then(
    () => true,
    () => false,
  );
}

function cleanInstallerLine(line: string): string {
  const compact = line.replace(/\s+/g, ' ').trim();
  return compact.length > 150 ? `${compact.slice(0, 147)}...` : compact;
}

async function verifyPythonInstaller(installerPath: string): Promise<void> {
...
export class RuntimeManager {
  ...
  private events: RuntimeProgress[] = [];
  private listeners = new Set<(event: RuntimeProgress) => void>();

  constructor(projectRoot: string) {
    this.runtimeRoot = resolve(
      process.env.MOTIONSCORE_RUNTIME_ROOT?.trim() ||
  ...
  private emit(event: RuntimeProgress): void {
    this.events.push(event);
    for (const listener of this.listeners) listener(event);
  }

  private candidatePython(): string | null {
    ...
    if (existsSync(this.managedPython)) return this.managedPython;
    return null;
  }

  private applyEnvironment(pythonPath: string, ffmpegPath?: string): void {
    ...
    if (usableFfmpeg) process.env.IMAGEIO_FFMPEG_EXE = usableFfmpeg;
  ...
  private async probe(pythonPath: string, deep = false): Promise<ProbePayload> {
    ...
    scriptLines.push(
      ...
      '  payload["ffmpegPath"] = imageio_ffmpeg.get_ffmpeg_exe()',
      'except Exception:',
      '  pass',
    ...
    if (!line) throw new Error('Runtime verification returned no version information.');
    return JSON.parse(line) as ProbePayload;
  }

  async inspect(): Promise<RuntimeStatus> {
    ...
    try {
      while (true) {
        ...
        this.emit({
          ...
          message:
            total > 0
              ? `Downloading private Python (${Math.round(received / 1_048_576)} of ${Math.round(
                  total / 1_048_576,
                )} MB)`
  ...
  async install(mode: RuntimeMode): Promise<void> {
    if (this.installing) throw new Error('Runtime installation is already in progress.');
  ...
  readMarker(): unknown {
    ...
    try {
      return JSON.parse(readFileSync(this.markerPath, 'utf8')) as unknown;
    } catch {
      return null;
    }
  }
