...
- Impact "juice" (Phase 2 DONE, VERIFIED): impact-reactive effects in `stream-render.ts` draw helpers, tuned by constants at top of file. (B1) squash & stretch — ball elongates along velocity (per-frame pos delta) between hits, flattens vertically on impact (squash keyed to each voice's own `impactTimes` via a monotonic `impactCursor`; ellipse). (B2) impact bloom (radial white->color->transparent gradient, `lighter`) + expanding shockwave rings BEHIND ball, and additive debris shards IN FRONT (stateless `pos=impact+dir*speed*age+0.5*g*age^2`, decorative gravity, deterministic `rand01` jitter). (B3) struck-key flash — white overlay over the key rect for `KEY_PRESS_SEC`. (B5) screen shake on strong impacts (`impactSize>=SHAKE_MIN_IMPACT`), whole-frame `ctx.translate` inside per-frame save/restore; static layer OVERSCANNED so shake never exposes an edge. (B7) additive (`lighter`) trail glow. Impacts are `sort`ed by time once; a per-frame sliding window `[impactLo,impactHi)` (bounded by `maxEffectWindow`) means effects scan only live impacts (O(active)), important for dense stems. bloom/shards/shake gated by `renderConfig.particlesOnImpact`; keypress + squash/stretch always on. VERIFIED: tsc/vitest 110-110/1080p 5-ball render clean + valid, render+encode ~32s (~+10% vs pre-juice), colors OK. Section cues (B4) NOT wired to renderer (bigger plumbing) — deferred.
